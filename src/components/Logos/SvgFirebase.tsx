import React from 'react';

const SvgFirebase = (props: any) => (
  <svg width="1em" height="1em" viewBox="0 0 111 151" {...props}>
    <defs>
      <linearGradient
        x1="-3.482%"
        y1="17.027%"
        x2="100.833%"
        y2="92.887%"
        id="firebase_svg__b"
      >
        <stop stopColor="#FFF" stopOpacity={0.1} offset="0%" />
        <stop stopColor="#FFF" stopOpacity={0} offset="100%" />
      </linearGradient>
      <path
        d="M95.798 33.625a3.436 3.436 0 00-2.336-2.699 3.454 3.454 0 00-3.48.823l-17.71 17.816-14.03-26.51A3.34 3.34 0 0055.25 21.2a3.34 3.34 0 00-2.995 1.854l-7.613 14.492L25.392 1.914c-1.54-2.898-5.903-2.131-6.416 1.108L.498 121.17l49.532 27.704a10.432 10.432 0 0010.01 0L110 121.17 95.798 33.625z"
        id="firebase_svg__a"
      />
    </defs>
    <g transform="translate(.193 -.022)" fill="none" fillRule="evenodd">
      <mask id="firebase_svg__c" fill="#fff">
        <use xlinkHref="#firebase_svg__a" />
      </mask>
      <use
        fill="url(#firebase_svg__b)"
        fillRule="nonzero"
        xlinkHref="#firebase_svg__a"
      />
      <g mask="url(#firebase_svg__c)">
        <path
          d="M.498 121.17L19.062 3.022c.513-3.239 4.876-4.006 6.416-1.108L44.64 37.546l7.613-14.492a3.423 3.423 0 013.037-1.838c1.278 0 2.449.709 3.037 1.838L110 121.17H.498z"
          fill="#FFA000"
          fillRule="nonzero"
        />
        <g transform="translate(-6.346 29.96)">
          <image
            opacity={0.12}
            width={74.427}
            height={94.62}
            xlinkHref="data:image/png;base64,"
          />
          <path
            fill="#F57C00"
            fillRule="nonzero"
            d="M70.834 45.179L50.901 7.587 6.844 91.21z"
          />
        </g>
        <g transform="translate(-2.924 23.14)">
          <image
            opacity={0.2}
            width={119.768}
            height={131.275}
            xlinkHref="data:image/png;base64,"
          />
          <path
            d="M112.924 98.03L98.723 10.485a3.436 3.436 0 00-2.337-2.698 3.454 3.454 0 00-3.48.823L3.421 98.03l49.532 27.704a10.432 10.432 0 0010.01 0l49.96-27.704z"
            fillRule="nonzero"
            fill="#FFCA28"
          />
        </g>
        <path
          d="M95.798 33.625a3.436 3.436 0 00-2.336-2.699 3.454 3.454 0 00-3.48.823l-17.71 17.816-14.03-26.51A3.34 3.34 0 0055.25 21.2a3.34 3.34 0 00-2.995 1.854l-7.613 14.492L25.392 1.914c-1.54-2.898-5.903-2.131-6.416 1.108L.498 121.17H.412l.086.085.513.255 71.176-71.092 17.794-17.731a3.453 3.453 0 013.48-.823 3.436 3.436 0 012.337 2.698l14.03 86.693.172-.085-14.202-87.545zM.668 120.999L19.063 3.875c.513-3.24 4.876-4.007 6.416-1.108L44.64 38.398l7.613-14.491a3.423 3.423 0 013.037-1.839c1.278 0 2.449.71 3.037 1.839L72.016 49.82.67 120.999z"
          fillOpacity={0.2}
          fill="#FFF"
          fillRule="nonzero"
        />
        <path
          d="M60.04 148.021a10.432 10.432 0 01-10.01 0L.583 120.402l-.085.768 49.532 27.704a10.432 10.432 0 0010.01 0L110 121.17l-.086-.768-49.875 27.62z"
          fill="#A52714"
          fillRule="nonzero"
          opacity={0.2}
        />
      </g>
      <path
        mask="url(#firebase_svg__c)"
        d="M-26.878-6.695h164.253v163.667H-26.878z"
      />
      <path
        d="M95.798 33.625a3.436 3.436 0 00-2.336-2.699 3.454 3.454 0 00-3.48.823l-17.71 17.816-14.03-26.51A3.34 3.34 0 0055.25 21.2a3.34 3.34 0 00-2.995 1.854l-7.613 14.492L25.392 1.914c-1.54-2.898-5.903-2.131-6.416 1.108L.498 121.17l49.532 27.704a10.432 10.432 0 0010.01 0L110 121.17 95.798 33.625z"
        fill="url(#firebase_svg__b)"
        fillRule="nonzero"
        mask="url(#firebase_svg__c)"
      />
    </g>
  </svg>
);

export default SvgFirebase;
